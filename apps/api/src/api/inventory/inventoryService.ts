...
export const inventoryService = {
  ...
  updateById: async (
    ...
    try {
      ...
      if (!updateResult) {
        return buildNotFoundServiceResponse(
          `No Inventory Item Found ID: [${id}]`,
        );
      }
  ...
  deleteById: async (
    ...
    try {
      const item = await deleteInventoryById(id);

      return buildSuccessServiceResponse("Delete Inventory", item);
    } catch (error) {
      const errorMsg = `Error while deleting inventory: ${(error as Error).message}`;
      return buildInternalErrorServiceResponse(errorMsg);
    }
  },
